...
1. 拉取笔记详情
2. 读取 `voiceRealtimeSessions[]`
3. 提取每个 session 下的 `cards[]`
4. 按 `sessionId + segmentIndex` 组织展示顺序
5. 详情区只展示分段卡片，不再渲染“详情聚合”容器
6. 如需播放音频，使用文件卡或播放器入口，不在详情区额外展示文件/会话聚合块
...
- 可保留独立文件卡用于播放、下载或状态展示
- 不渲染“详情聚合”容器，不单独渲染文件区/会话区摘要
- 详情主体直接展示每个 session 下的 `cards[]`
- 如果有多个 session，按 session 分组或按时间顺序平铺卡片都可以，但最终只保留分段卡片视图
...
14. 从 `voiceRealtimeSessions[]` 提取 `cards[]`
15. 详情区只展示分段卡片
...
2. 上传只做上传，不做转写。
3. 实时转写不先落文件。
4. 详情接口可以按 session 返回，但详情区前端只消费 cards。
...
`detail -> voiceRealtimeSessions[] -> cards[]`
